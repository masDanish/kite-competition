...
import AnnouncementItem from '@/Pages/User/AnnouncementItem';
import {
...
import {
    ...
    ChevronRight, ChevronLeft, ChevronDown, Rocket, ArrowDown
} from 'lucide-react';
...
const FLOW_STEPS = [
    { step:1, icon:CalendarPlus, label:'Daftar Event',   desc:'Pilih event & kategori',       grad:'from-indigo-500 to-blue-600',   shadow:'shadow-indigo-300',  userAction:true, actionLabel:'Daftar', actionRoute:'events.index',             emoji:'🎯' },
    { step:2, icon:UserCheck,    label:'Admin Approve',  desc:'Tunggu konfirmasi admin',       grad:'from-violet-500 to-purple-600', shadow:'shadow-violet-300',  waiting:true,                                                                    emoji:'⏳' },
    { step:3, icon:FileUp,       label:'Upload Karya',   desc:'Unggah karya terbaikmu!',       grad:'from-rose-500 to-pink-600',     shadow:'shadow-rose-300',    userAction:true, actionLabel:'Upload', actionRoute:'user.registrations.index', emoji:'🎨' },
    { step:4, icon:Star,         label:'Juri Menilai',   desc:'Juri profesional menilai',      grad:'from-amber-500 to-orange-500',  shadow:'shadow-amber-300',   waiting:true,                                                                    emoji:'⭐' },
    { step:5, icon:Trophy,       label:'Pengumuman',     desc:'Pantau hasilnya!',              grad:'from-emerald-500 to-teal-600',  shadow:'shadow-emerald-300', highlight:true,                                                                   emoji:'🏆' },
    { step:6, icon:Gift,         label:'Hadiah',         desc:'Pemenang dapat hadiah',         grad:'from-yellow-400 to-amber-500',  shadow:'shadow-yellow-300',  highlight:true,                                                                   emoji:'🎁' },
];
    ...
    return (
        <UserLayout header="Dashboard">
            ...
            <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} transition={{duration:0.7}}
                ...
                <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-5">
                    <div>
                    ...
                    <motion.div animate={{rotate:[0,8,-4,0],y:[0,-10,0]}} transition={{duration:5,repeat:Infinity,ease:'easeInOut'}}
                        ...
                        <svg width="72" height="86" viewBox="0 0 100 120" fill="none">
                            <defs>
                    ...
                    <motion.div key={i} variants={fadeUp}>
                        <TiltCard className="group" intensity={14}>
                            <motion.div
                                ...
                                whileHover={{y:-4}}
                                className={`relative bg-white rounded-xl sm:rounded-2xl p-3 sm:p-5 shadow-lg ${s.shadow}
                                            border border-gray-100 flex items-center gap-2 sm:gap-4 overflow-hidden cursor-default`}>
                                ...
                                <motion.div className={`w-9 h-9 sm:w-12 sm:h-12 bg-gradient-to-br ${s.grad} rounded-lg sm:rounded-xl
                                ...
                                <div className="min-w-0">
                                    <motion.p className="text-xl sm:text-2xl font-black text-gray-800 tabular-nums"
                                ...
                                <div className={`absolute -bottom-3 -right-3 w-12 h-12 sm:w-16 sm:h-16
            ...
            <motion.div initial={{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.15}}
                className="mb-5 sm:mb-8 rounded-2xl sm:rounded-3xl overflow-hidden border border-gray-100 shadow-sm bg-white">
                    ...
                    <div className="relative flex items-center justify-between flex-wrap gap-3">
                        <div className="flex items-center gap-3">
                            <motion.div animate={{rotate:[0,15,-10,0]}} transition={{duration:4,repeat:Infinity}}
                                className="w-8 h-8 sm:w-9 sm:h-9 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center">
                                <Rocket className="w-4 h-4 text-white"/>
                            </motion.div>
                            <div>
                                <h2 className="font-black text-white text-sm sm:text-base">Alur Kompetisi</h2>
                                <p className="text-indigo-200 text-[10px] sm:text-xs">6 langkah untuk menjadi juara 🏆</p>
                            </div>
                        </div>
                        <motion.span animate={{scale:[1,1.05,1]}} transition={{duration:2,repeat:Infinity}}
                            className="inline-flex items-center gap-1.5 bg-white/20 border border-white/30
                                       text-white text-xs font-bold px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full backdrop-blur-sm">
                ...
                <div className="p-4 sm:p-6">
                    ...
                    {/* ── Desktop flow — snake layout ── */}
                    <div className="hidden lg:block">

                        {/* Row 1: steps 1→2→3→4 (left to right) */}
                        <div className="flex items-stretch gap-0">
                            {FLOW_STEPS.slice(0,4).map((s,i) => (
                                <FlowStep3D key={s.step} step={s} isLast={i===3} arrowDir="right" index={i}/>
                            ))}
                        </div>

                        {/* U-Turn connector: animated arrow going DOWN on the right side */}
                        <div className="relative flex justify-end pr-[calc(12.5%-28px)] my-1">
                            <UTurnConnector />
                        </div>

                        {/* Row 2: steps 5→6 displayed right-to-left visually, but data order correct */}
                        {/* We render step 6 first then 5 in normal flex, so visually: [6][5] reversed */}
                        {/* Actually: render in reverse order so step 5 is on the right, step 6 on left */}
                        <div className="flex items-stretch gap-0">
                            {/* empty spacer to align with row1 */}
                            <div className="flex-1 flex items-center">
                                <div className="flex-1 border-t-2 border-dashed border-gray-100 rounded"/>
                            </div>
                            {/* Step 6 then step 5, with arrow pointing LEFT between them */}
                            {[FLOW_STEPS[5], FLOW_STEPS[4]].map((s,i) => (
                                <FlowStep3D key={s.step} step={s} isLast={i===1} arrowDir="left" index={i+4}/>
                            ))}
                        </div>
                    </div>

                    {/* ── Tablet flow — 3+3 grid ── */}
                    <div className="hidden sm:grid lg:hidden grid-cols-3 gap-3">
                        {FLOW_STEPS.map((s,i)=>(
                            <FlowStepGrid key={s.step} step={s} index={i}/>
                        ))}
                    </div>
                ...
                <motion.div initial={{opacity:0,x:-24}} animate={{opacity:1,x:0}} transition={{duration:0.6,delay:0.2}}
                    ...
                    <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="font-bold text-gray-800 text-sm sm:text-base">Pendaftaran Saya</h2>
                        </div>
                    ...
                    <div className="p-4 sm:p-6">
                        {registrations.length === 0 ? (
                            <motion.div initial={{opacity:0}} animate={{opacity:1}} className="text-center py-8 sm:py-10">
                                ...
                                <Link href={route('events.index')}
                                    className="inline-flex items-center gap-1.5 bg-gradient-to-r from-indigo-600 to-blue-600
                                               text-white text-sm font-bold px-4 sm:px-5 py-2.5 rounded-xl shadow-md shadow-indigo-200
                            ...
                            <div className="space-y-2 sm:space-y-3">
                                ...
                                {registrations.length > 4 && (
                                    <Link href={route('user.registrations.index')}
                                        ...
                                        +{registrations.length-4} lainnya →
                                    </Link>
                                )}
                            </div>
                        )}
                ...
                <motion.div initial={{opacity:0,x:24}} animate={{opacity:1,x:0}} transition={{duration:0.6,delay:0.3}}
                    ...
                    <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <h2 className="font-bold text-gray-800 text-sm sm:text-base">Pengumuman</h2>
                                {announcements.length > 0 && (
                                    <p className="text-[10px] text-gray-400 hidden sm:block">Klik untuk lihat detail</p>
                                )}
                            </div>
                        </div>
                        {announcements.length > 0 && (
                    ...
                    <div className="p-3 sm:p-4 flex-1 overflow-y-auto max-h-[360px] sm:max-h-[480px]">
                        {announcements.length === 0 ? (
...
/* ══════════════════════════════════════════════════════
   U-Turn Connector (animated snake bend)
══════════════════════════════════════════════════════ */
function UTurnConnector() {
    return (
        <div className="relative w-14 h-10 flex items-center justify-center">
            {/* curved path SVG */}
            <svg width="56" height="40" viewBox="0 0 56 40" fill="none" className="absolute">
                <motion.path
                    d="M4 4 Q52 4 52 20 Q52 36 4 36"
                    stroke="url(#uturn-grad)" strokeWidth="2.5" strokeLinecap="round"
                ...
                />
                <defs>
                    <linearGradient id="uturn-grad" x1="0" y1="0" x2="1" y2="1">
                        <stop offset="0%" stopColor="#6366f1"/>
                        <stop offset="100%" stopColor="#06b6d4"/>
                    </linearGradient>
                </defs>
            </svg>
            {/* animated arrow tip pointing LEFT (←) at the end */}
            <motion.div
                className="absolute left-0 top-1/2 -translate-y-1/2 flex items-center justify-center
                           w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 shadow-md shadow-indigo-200"
                animate={{ x: [0, -3, 0] }}
...
/* ══════════════════════════════════════════════════════
   Flow Step — Desktop
   arrowDir: 'right' | 'left'  controls arrow orientation
══════════════════════════════════════════════════════ */
function FlowStep3D({ step, isLast, arrowDir = 'right', index }) {
    const Icon = step.icon;
    const [hovered, setHovered] = useState(false);

    const ArrowIcon = arrowDir === 'right' ? ChevronRight : ChevronLeft;
    // For left-direction rows the connector should appear BEFORE the card (leading arrow)
    const connectorBefore = arrowDir === 'left' && !isLast;
    const connectorAfter  = arrowDir === 'right' && !isLast;
    ...
    return (
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}
            ...
            {/* connector BEFORE card (left-direction rows) */}
            {connectorBefore && <FlowConnector dir={arrowDir} index={index}/>}
            ...
            <TiltCard intensity={16} className="flex-1 min-w-0">
                <motion.div
                    ...
                    <AnimatePresence>
                        {hovered && (
                            <motion.span key="float"
                                className="absolute top-1 right-2 text-base pointer-events-none"
                    ...
                    <motion.div className={`relative w-12 h-12 bg-gradient-to-br ${step.grad} rounded-2xl
                        ...
                        <span className="absolute -top-2 -right-2 w-4 h-4 bg-white border-2 border-gray-200
                                         rounded-full text-[9px] font-black text-gray-600
                    ...
                    <p className="text-[10px] text-gray-400 leading-tight max-w-[80px]">{step.desc}</p>
                    <div className="mt-2">
                        {step.userAction && (
                            <Link href={route(step.actionRoute)}
                                className="inline-flex items-center gap-1 bg-gradient-to-r from-indigo-600
                                           ...
                                           rounded-lg shadow-sm shadow-indigo-200 hover:-translate-y-0.5 transition-all duration-200">
            ...
            {/* connector AFTER card (right-direction rows) */}
            {connectorAfter && <FlowConnector dir={arrowDir} index={index}/>}
        </motion.div>
    );
}

/* Animated connector with correct arrow direction */
function FlowConnector({ dir, index }) {
    const ArrowIcon = dir === 'right' ? ChevronRight : ChevronLeft;
    return (
        <motion.div
            initial={{scaleX:0,opacity:0}} animate={{scaleX:1,opacity:1}}
            transition={{delay:index*0.08+0.35,duration:0.4}}
            style={{transformOrigin: dir==='right' ? 'left' : 'right'}}
            className="flex items-center shrink-0 mx-0.5 gap-0">
            {dir === 'left' && (
                <motion.div
                    animate={{x:[0,-3,0]}} transition={{duration:1,repeat:Infinity,ease:'easeInOut'}}
                    className="flex items-center">
                    <ArrowIcon className="w-4 h-4 text-indigo-400 -mr-1"/>
                </motion.div>
            )}
            <div className={`w-4 h-0.5 bg-gradient-to-r
                ${dir==='right' ? 'from-indigo-300 to-blue-300' : 'from-blue-300 to-indigo-300'}
                rounded`}/>
            {dir === 'right' && (
                <motion.div
                    animate={{x:[0,3,0]}} transition={{duration:1,repeat:Infinity,ease:'easeInOut'}}
                    className="flex items-center">
                    <ArrowIcon className="w-4 h-4 text-indigo-400 -ml-1"/>
                </motion.div>
            )}
        </motion.div>
    );
...
/* ══════════════════════════════════════════════════════
   Flow Step — Tablet Grid
══════════════════════════════════════════════════════ */
function FlowStepGrid({ step, index }) {
    const Icon = step.icon;
    return (
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}}
            transition={{delay:index*0.07,duration:0.4}}
            className="flex flex-col items-center text-center p-3 rounded-2xl border border-gray-100 bg-white shadow-sm">
            <div className={`relative w-11 h-11 bg-gradient-to-br ${step.grad} rounded-xl
                             flex items-center justify-center shadow-md ${step.shadow} mb-2`}>
                <Icon className="w-5 h-5 text-white"/>
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-white border border-gray-200
                                 rounded-full text-[9px] font-black text-gray-600 flex items-center justify-center">{step.step}</span>
            </div>
            <p className="text-xs font-bold text-gray-800 mb-0.5">{step.label}</p>
            ...
            {step.waiting && (
                <span className="inline-flex items-center gap-1 bg-gray-100 text-gray-500 text-[10px] font-semibold px-2 py-1 rounded-lg">
                    <Clock className="w-2.5 h-2.5"/> Menunggu
                </span>
            )}
            {step.highlight && (
        ...
        <motion.div initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} transition={{delay:index*0.07}}
            ...
            <div className="flex flex-col items-center shrink-0">
                <motion.div whileHover={{scale:1.1,rotate:5}}
                    className={`relative w-9 h-9 bg-gradient-to-br ${step.grad} rounded-xl
                                 flex items-center justify-center shadow-md ${step.shadow}`}>
                    <Icon className="w-4 h-4 text-white"/>
                    <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-white border border-gray-200
                                     rounded-full text-[8px] font-black text-gray-600 flex items-center justify-center">
                        {step.step}
                    </span>
                </motion.div>
                {!isLast && (
                    <div className="flex flex-col items-center mt-1 gap-0.5">
                        <motion.div className="w-px bg-gradient-to-b from-indigo-300 to-transparent"
                            initial={{height:0}} animate={{height:16}} transition={{delay:index*0.07+0.3}}/>
                        <motion.div
                            animate={{y:[0,3,0]}} transition={{duration:0.8,repeat:Infinity,ease:'easeInOut'}}>
...
function RegistrationItem({ reg, index }) {
    ...
    return (
        <motion.div initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} transition={{delay:index*0.07}}
            whileHover={{x:4}}
            className="flex items-center gap-2 sm:gap-3 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-gray-100
            ...
            <motion.div animate={{scale:[1,1.4,1]}} transition={{duration:2,repeat:Infinity,delay:index*0.3}}
                className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full shrink-0 ${cfg.dot}`}/>
            <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800 text-xs sm:text-sm truncate">{reg.event.title}</p>
                <p className="text-[10px] sm:text-xs text-gray-400 truncate">{reg.category.name}</p>
            </div>
                ...
                {isApproved && !hasSubmission ? (
                    <Link href={route('user.submissions.create', reg.id)}
                        className="flex items-center gap-1 bg-gradient-to-r from-indigo-600 to-blue-600
                                   text-white text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg shadow-sm
                    ...
                    <span className="flex items-center gap-1 text-[10px] sm:text-xs font-bold text-emerald-700
                                     bg-emerald-100 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg border border-emerald-200">
                        ✅ Upload
                    </span>
                ) : (
                    <span className={`text-[10px] sm:text-xs font-semibold px-2 sm:px-2.5 py-1 rounded-lg ${cfg.bg} ${cfg.text}`}>
                        {cfg.label}
                    </span>
                )}
            </div>
