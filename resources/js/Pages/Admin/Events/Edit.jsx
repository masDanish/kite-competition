            ...
            <motion.div
                ...
                <div className="relative z-10 flex justify-between items-start gap-3">
                    <div className="min-w-0 flex-1">
                        ...
                        <h1 className="text-xl sm:text-2xl font-black">Edit Event ✏️</h1>
                        <p className="text-slate-300 text-xs sm:text-sm mt-1 truncate max-w-xs sm:max-w-sm">
                            {event.title}
                        </p>
                    </div>
                    {/* Status badge — shown on sm+ */}
                    <div className={`hidden sm:flex items-center gap-2 px-3 sm:px-4 py-2 rounded-2xl
                                    font-bold text-xs sm:text-sm shrink-0 ${statusCfg.bg} ${statusCfg.text}`}>
            ...
            <form onSubmit={submit} className="w-full max-w-4xl space-y-4 sm:space-y-6">
                ...
                {event.categories?.length > 0 && (
                    <SectionCard icon={Tag} iconBg="bg-violet-100" iconColor="text-violet-600"
                            ...
                            {event.categories.map(cat => (
                                <span key={cat.id}
                                    className="flex items-center gap-1.5 bg-violet-50 text-violet-700
                                               text-xs sm:text-sm px-2.5 sm:px-3 py-1.5 rounded-full
                        ...
                        <p className="flex items-start gap-1.5 text-xs text-gray-400 bg-gray-50
